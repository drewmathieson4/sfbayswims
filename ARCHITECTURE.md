...
The app is a static site: `index.html` loads one ES module, `js/main.js`, which loads the rest. No build step,
no dependencies. Data lives under `data/`, prepared by the Python scripts under `tools/`. Everything is metres:
each world has a local origin, positions are metres east/north of it, and the SVG's user units are metres too
(so a 60 m swimmer glyph in the Bay is literally `dotR: 60`).

```
index.html ─ js/main.js ─┬─ js/world.js ─┬─ js/geometry.js / js/mask.js   grid + zones (cove) | water-mask PNG (bay)
                         │               ├─ js/routes.js                   waypoints → metre polylines, pier-following
                         │               ├─ js/photo.js · js/map.js        the aerial <image>; SVG layers
                         │               └─ js/current.js / js/stationfield.js   the two current fields
                         └─ js/app.js ───┬─ js/data.js · js/tide.js        live NOAA/USGS/NWS + bundles; tide extremes
                                         ├─ js/swim.js                     the physics: crab, sprint, sweep, 48-h scan
                                         ├─ js/animate.js · js/particles.js   the swimmer; the streaks
                                         └─ js/ui.js · js/ambient.js       HUD, rail, keys, kiosk; room-light dimming
         js/config.js · js/state.js · js/projection.js are leaves everyone imports.
```
...
**`css/style.css`** — one dark palette in `:root` (white ink on black; the photos are dark-toned and a dark ground
avoids a white flash on the nightly reload), the HUD/rail typography, the swimmer's classes, the `no-ui /
no-swimmer / no-streaks` switches and the kiosk cursor rule. Four custom properties are set from config at boot
(`--photo-filter`, `--route-done-w`, `--swimmer-r`, `--ui-scale`).

**`js/main.js`** — boot. Parses the URL flags (`urlOverrides` re-applies them after every world's config so a
flag always wins), sizes the view (`applyView`: one viewBox in metres shared by both SVGs, the canvas at device
resolution — re-run by a ResizeObserver), loads `data/worlds/index.json`, starts `app.js`, then `activate()`s the
first world (`?world=`, else the kiosk's remembered world, else the index default). `switchWorld()` /
`nextWorld()` (the `v` key) save the current route, load the other world's folder (cached), unmount, rebuild,
mount. The other world is preloaded 5 s after boot so the switch is instant. `window.APP` exposes state and the
...
**`js/projection.js`** — the local tangent plane (`createProjection(origin)`: lat/lon ↔ metres, x east, y north),
`fitView` (expand an extent to the screen's aspect), `toPx`, `viewBoxOf`, extent helpers.

**`js/geometry.js`** — zone worlds (the cove). Turns `shoreline.geojson` (OSM piers, breakwater, beach,
...
**`js/mask.js`** — mask worlds (the Bay): decodes `mask.png` (255 = water, 10 m cells) into the same grid contract.

**`js/routes.js`** — `routes.json` + `landmarks.json` → routes in metres. Waypoints are landmark ids, inline
...
**`js/tide.js`** — `TideSeries`: NOAA hi/lo extremes → the rate of rise/fall by cosine interpolation (what the
cove's fill/drain needs), `covers`, `merge` (bundle + cache + live).

**`js/current.js`** — `CurrentSeries` (station vectors over time, from live samples or a compact year bundle) and
...
**`js/stationfield.js`** — the **Bay field**, same interface: per lattice cell the 5 nearest stations and their
...
**`js/swim.js`** — the physics. `integrateLeg`: 10 m steps; at each, the crab solution `g = √(v² − c⊥²) + c∥`;
...
**`js/app.js`** — the running app. Live data: bundles → localStorage cache → NOAA/USGS/NWS refresh loops (tides,
...
**`js/photo.js`** — the aerial: places the `<image>` from `photo.json`'s bbox in metres and picks the default variant.

**`js/map.js`** — `el()` for namespaced SVG elements and the two layer groups (`#route`, `#swimmer`).

**`js/data.js`** — every network call (NOAA CO-OPS tides/currents, USGS water temperature with fallbacks, NWS /
