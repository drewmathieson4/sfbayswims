> Historical document from before the September 2026 maintenance pass. See the root documents for current behavior.
...
one per cycle (the fastest within ±half a cycle), filtered (daylight via `sun.js`, duration, weekends), with `slackNear`
