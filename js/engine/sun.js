...
function crossings(dayNoonMs, lat, lon, altitudeDeg) {
  const n = Math.round(jd(dayNoonMs) - 2451545 + 0.0008 - lon / 360), Js = n - lon / 360;
...
/** { dawn, sunrise, sunset, dusk } for the day containing `t` (use local noon for `t` to be safe). */
export function sunTimes(t, lat, lon) {
  const day = crossings(t, lat, lon, -0.833), civil = crossings(t, lat, lon, -6);
