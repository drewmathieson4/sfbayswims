...
const DEFAULTS = {
  ...
  swimSeconds: 20, holdSeconds: 1, fadeSeconds: 1, restSeconds: 2, sweptSeconds: 6, skipInfeasible: false, streaksFollowSwimmer: true,
  routes: {}, crumbsPerSwim: {}, tailFrac: 0.15, icon: 'glyph', streakAlpha: 0.18, maxFps: 30,
...
export async function loadPresets(params) {
  ...
  for (const k of ['swimSeconds', 'restSeconds', 'sweptSeconds', 'alternateEveryMin', 'maxFps', 'streakAlpha']) if (params.has(k)) p[k] = +params.get(k);
